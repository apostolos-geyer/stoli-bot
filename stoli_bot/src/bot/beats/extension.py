...
from stoli_bot.src.services.notion import crud as notion_crud
...
CREATE_PROFILE_MODAL: Callable[[], Modal] = lambda: (
    Modal(
        ShortText(
...
class Beats(Extension):
    ...
    async def create_profile(self, ctx: SlashContext):
        ...
        if producer_profile := await get_producer_from_discord_id(discord_id=discord_id):
            await ctx.send(
        ...
        if verify_created := await get_producer_from_discord_id(discord_id=discord_id):
            return await modal_ctx.send(f'User {ctx.user.mention} registered: {producer_entry}')

        raise Exception('An error occurred or something lol not my problem tbh')

    @profile_group.subcommand(sub_cmd_name='check', sub_cmd_description='Check your producer profile')
    async def check_profile(self, ctx: SlashContext):
        ...
        if producer_profile := await get_producer_from_discord_id(discord_id=discord_id):
            ...
            return await ctx.send(f"-_- u need to register first {ctx.user.mention}.. \n use /beats profile register :333 then i can have ur INFORMATION ")
    ...
    async def send_beat(
        ...
        if (producer_profile := await get_producer_from_discord_id(discord_id=discord_id)) is None:
            await ctx.send(f'{ctx.user.mention} you must register your producer profile first. Use /profile check')
            return

        producer_name: str = producer_profile.name

        try:
            beat_entry = Beat(title=title, producer_name=producer_name, bpm=bpm, key=key, detune=detune, link=url)
            result = await notion_crud.notion_create(beat_entry)
            ...
            return await ctx.send(f'somefing went wrong daddy im sowwy o.o.. pwease dont punish me\n {e}')
